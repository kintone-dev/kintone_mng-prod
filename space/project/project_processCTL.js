...
async function linkage_shipData(event){
  // 入出荷管理post用配列初期化
  let postShipData = {'app': sysid.INV.app_id.shipment, 'records': []};
  // 入出荷管理post内容_通常
  let postShipBody = {
    'prjId': {'value': event.record.$id.value},
    'shipType': {'value': '移動-'+event.record.salesType.value},
  ...
  let postShipSubBody = {
    ...
    'deviceList': {'value': []},
    'sys_prjId': {'value': event.record.$id.value + '-sub'}
  }
  for(let i in event.record.deviceList.value){
  ...
  if(event.record.sys_shipment_ID.value){
    // postBodyに更新用IDを追加
    // await kintone.api(kintone.api.url('/k/v1/records', true), "PUT", postShipData);
    event.error = '連携済みデータです。'
  }else{
    let linkage_shipment = await kintone.api(kintone.api.url('/k/v1/records', true), "POST", postShipData);
    event.record.sys_shipment_ID.value = linkage_shipment;
  }
  return event;
}
